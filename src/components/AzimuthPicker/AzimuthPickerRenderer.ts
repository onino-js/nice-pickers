...
const valueToAngle = (value: number) => value + 180;
...
export const cardinalPointPickerRender = (
  container: any,
  value: number,
  onChange: (a: number) => void,
  possibleValues: AngleValue[] = baseAngles,
  cardinals: AngleValue[] = [
    { label: 'N', angle: 0 },
  ...
  const angleToValue = (angle: number): AngleValue =>
    possibleValues.reduce(
      (a: AngleValue, b: AngleValue) =>
        Math.abs(a.angle - angle) < Math.abs(b.angle - angle) ? a : b,
      possibleValues[0]
    );
  ...
  let dragEnd = () => {
    ...
    d3.select(container)
      ...
      .attr(
        'transform',
        `rotate(${valueToAngle(angleToValue(pvGisAngle).angle)})`
      );
  ...
  d3.select(container)
    ...
    .attr(
      'viewBox',
      ` 0 0 ${chartHeight + padding.top + padding.bottom} ${chartWidth +
        padding.left +
        padding.right}`
    )
    ...
    .attr(
      'transform',
      `translate(${padding.left + chartWidth / 2},${padding.top +
        chartHeight / 2})`
    )
  ...
  gCardinals
    ...
    .attr(
      'x',
      // @ts-ignore
      d => textArc.centroid({ startAngle: d.angle, endAngle: d.angle })[0]
    )
    .attr(
      'y',
      // @ts-ignore
      d => textArc.centroid({ startAngle: d.angle, endAngle: d.angle })[1]
    )
    .attr('dy', '0.5em')
    // @ts-ignore
    .merge(gCardinals)
    .attr('class', (d: any) =>
      d.angle === valueToAngle(value) * (Math.PI / 180)
        ? 'cardinal selected'
        : 'cardinal'
    );
